...
const Esteira = ({step, setStep, labels}) => {
    ...
    return (
        <Box display={"flex"} mb={5} gap={1}>
            {
                labels.map((label, i) => (
                    <span key={i} style={{cursor: (i >= step-1 && i <= step+1) ? "pointer" : "default", borderRadius: "50%"}} >
                        <EsteiraItem label={label} handleClick={handleClick} step={step} atual={i} ativo={step >= i}/>
                    </span>
                ))
            }
        </Box>
...
const EsteiraItem = ({ativo, atual, step, handleClick, label}) => {
    ...
    return (
        <Box display={"flex"} alignItems={"center"}>
            {atual !== 0 && <Line color={ativo ? theme.palette.secondary.light : "#e1e1e1"} />}
            <Box position={"relative"} display={"flex"} width={30} flexDirection={"column"} alignItems={"center"}>
                {atual === step ? (
                ...
                <Typography sx={{width: 200}} mt={5} ml={20} position={"absolute"} color={atual === step ? "black" : "gray"}>{label}</Typography>
            </Box>
        </Box>
    );
}
